...
export async function getAdminStats() {
    ...
    const [locations, users, engagement, payments] = await Promise.all([
    ...
    return { locations: locations.data, users: users.data, engagement: engagement.data, payments: payments.data }
}

export async function getRecentLocations(limit = 5) {
    if (!supabase) return mockRecentLocations
    const { data } = await supabase.from('locations').select('id, title, category, city, created_at').order('created_at', { ascending: false }).limit(limit)
    return data || []
}

export async function getRecentActivity(limit = 10) {
    if (!supabase) return mockRecentActivity
    const { data } = await supabase.from('user_visits').select('id, user_id, location_id, visited_at, locations(title)').order('visited_at', { ascending: false }).limit(limit)
    return data || []
}
...
export async function getPendingLocations() {
    ...
    const { data } = await supabase.from('locations').select('id, title, category, city, created_at').eq('status', 'pending').order('created_at', { ascending: false })
    return data || []
}

// Mock data for development without Supabase
const mockAdminStats = { locations: { total: 0, published: 0, pending: 0, rejected: 0 }, users: { total: 0, this_month: 0, this_week: 0 }, engagement: { total_visits: 0, total_reviews: 0, total_favorites: 0, pending_reviews: 0 }, payments: { total_payments: 0, total_revenue: 0, active_subscriptions: 0, this_month_revenue: 0 } }
