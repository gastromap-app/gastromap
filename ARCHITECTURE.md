...
| Zustand | UI-only state (modals, scroll positions, theme). Server data uses React Query exclusively. |
| React Query (TanStack) | Серверный кэш, мутации — ЕДИНСТВЕННЫЙ источник истины для серверных данных |
...
| Vercel Serverless Functions | AI proxy (`/api/ai/chat`), KG save (service_role) |
| Supabase Edge Functions | Utility functions (NOT used for AI chat) |
...
### 6.4 Логика фильтрации (URL-driven)

> **Обновлено 2026-05-16:** `useLocationsStore` удалён. Фильтры теперь управляются через URL.

Filters are now URL-driven via `useLocationFilters()` hook. Server-side filtering via Supabase queries.

```js
// src/hooks/useLocationFilters.js
// Читает/пишет фильтры в URL searchParams (выживает refresh, shareable)
const { filters, setFilter, resetFilters } = useLocationFilters()
// Компоненты используют useLocationsInBounds(bounds, filters) для карты
```
...
**Endpoint**: Primary proxy is `/api/ai/chat` (Vercel serverless). Supabase Edge Function `ai-chat` is NOT deployed.  
**Модель**: OpenRouter (GPT-class primary, Llama fallback)

> **Обновлено 2026-05-16:** Paid models use `_direct_model` mode (no cascade). Admin cascade config (`_cascade`) is passed to server and respected.
