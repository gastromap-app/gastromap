...
export const supabase = config.supabase.isConfigured
    ? createClient(config.supabase.url, config.supabase.anonKey, {
        auth: {
            ...
            // CRITICAL: Bypass Web Locks API to prevent deadlock.
            // Supabase fires onAuthStateChange INSIDE the lock, and our
            // callback calls _fetchProfile() → getSession() which tries
            // to re-acquire the same lock → deadlock → login freeze.
            // The no-op lock avoids this by never actually locking.
            // Trade-off: multi-tab token refresh is uncoordinated, but
            // that's acceptable — the alternative is a frozen UI.
            lock: async (_name, _acquireTimeout, fn) => fn(),
        },
        global: {
            // 10-second timeout on all Supabase fetch requests.
            // Supabase free tier may throttle — shorter timeout + retry is better
            // than waiting forever. 10s balances between giving Supabase time
