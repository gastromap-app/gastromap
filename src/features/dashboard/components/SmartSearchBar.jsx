        ...
        <div ref={containerRef} className={`relative flex gap-2 ${className}`}>
            ...
            <div
                className={`flex-1 relative flex items-center h-12 px-4 rounded-2xl transition-all border ${
                    isDark
                        ? `bg-white/5 border-white/10 ${focused ? 'border-blue-500/40 shadow-lg shadow-blue-500/10' : ''}`
                ...
                <AnimatePresence>
                    {value && (
                        <motion.button
                            ...
                            className={`ml-2 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${isDark ? 'bg-white/10 text-white' : 'bg-gray-100 text-gray-500'}`}
                        >
                            <X size={11} />
                        </motion.button>
                    )}
            ...
            >
                ...
                {useLocationsStore.getState().getActiveFiltersCount() > 0 && (
                    <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center border-2 border-white dark:border-[#0f1117] animate-in fade-in zoom-in duration-300">
                    ...
                    >
                        {suggestions.map((loc, i) => (
                            <React.Fragment key={loc.id}>
                                {i > 0 && (
                                    <div className={`h-px mx-4 ${isDark ? 'bg-white/5' : 'bg-gray-50'}`} />
                                )}
                                <button
