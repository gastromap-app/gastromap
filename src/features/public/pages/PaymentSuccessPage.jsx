...
const PaymentSuccessPage = () => {
    ...
    useEffect(() => {
        // Here we could potentially call our backend to verify the session
        // but for now, we just display the success state.
        if (!sessionId) {
            navigate('/');
        }
    }, [sessionId, navigate]);
        ...
        <div className="min-h-screen bg-[#F5F5F7] dark:bg-black pt-24 sm:pt-32 pb-12 sm:pb-20 flex flex-col items-center justify-center px-4" style={{ paddingTop: 'max(6rem, env(safe-area-inset-top) + 4rem)' }}>
            <div className="bg-white dark:bg-[#1C1C1E] p-6 sm:p-8 md:p-12 rounded-2xl sm:rounded-3xl shadow-sm border border-black/5 dark:border-white/10 max-w-lg w-full text-center">
                <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center mx-auto mb-5 sm:mb-6">
                    <CheckCircle className="w-8 h-8 sm:w-10 sm:h-10 text-green-600 dark:text-green-500" />
                </div>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-base sm:text-lg mb-6 sm:mb-8">
                    {t('payment.success.description', 'Thank you for your support. Your contribution helps us keep GastroMap running and improving.')}
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                    ...
                    >
                        {t('payment.success.dashboard', 'Go to Dashboard')}
                    </button>
                    <button
                        onClick={() => navigate('/')}
                    ...
                    >
                        {t('payment.success.home', 'Back to Home')}
                    </button>
                </div>
            </div>
        </div>
